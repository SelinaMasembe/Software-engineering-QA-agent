...
**Generated:** 2026-09-25T20:06:05Z
**Documents:** 22
...
| `requirements/prompt-specification.txt` | `docs/prompts/Prompt_Specification.docx` | requirement | — | Member 3 (Week 2) | `b2d838bddbd4` | The full prompt specification behind propose_action, including the failure-behaviour contract the malformed-source-code case relies on. |
| `requirements/model-selection-note.txt` | `docs/prompts/Model_Selection_Note.docx` | requirement | — | Member 3 (Week 2) | `b425df7a6fe7` | The model selection rationale the ChatCompletionsClient is built against. |
...
| `code/rag_retrieval.py` | `src/rag/retrieval.py` | code | — | Member 2 (Week 3) | `ee1c7d268f6c` | The retrieval pipeline itself: indexes a corpus and makes the not_in_corpus decision in ordinary code, not the model. |
...
| `logs/week3-fifteen-case-rag-evaluation.md` | `docs/evaluation/week3-fifteen-case-rag-evaluation.md` | log | — | Member 4 (Week 3) | `26c60d018bf8` | The scored results of actually running fifteen retrieval cases against the live pipeline, including the four documented, reproduced retrieval-limitation cases. |
| `logs/offline-test-suite.txt` | `generated by running tests.test_config, tests.test_prompt_harness, tests.test_rag_eval, tests.integration.test_model_integration, tests.integration.test_rag_pipeline (see setup.md section 5)` | log | — | src/ingestion/tag_provenance.py | `813bcb68a043` | Captured output from the team's offline test suite. The artifact uses .txt because the sensitive-data guard blocks committed .log files. |
