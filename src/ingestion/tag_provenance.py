...
# (tests/fixtures/member2/corpus/logs/test_run_2026_09_15.txt) was produced by
...
def _collect_test_suite_log(
    ...
    """Capture the offline suite using a non-sensitive .txt corpus name."""
    ...
    header = (
        f"$ PYTHONPATH=src {' '.join(command)}\n"
        f"# captured {collected_at}\n\n"
    )
    footer = (
        f"\n# exit code {completed.returncode}: "
        f"{tests_run} tests run ({status_detail or 'all passed'})\n"
    )
    ...
    if completed.returncode != 0:
        print(
            ...
            "this corpus artifact.",
            file=sys.stderr,
        )

    return (
        ProvenanceRecord(
            source_path=f"{_TYPE_DIR_NAME[DocumentType.LOG]}/offline-test-suite.txt",
            original_path=(
            ...
            line_count=len(text.splitlines()),
            reason=(
                "Captured output from the team's offline test suite. "
                "The artifact uses .txt because the sensitive-data guard "
                "blocks committed .log files."
            ),
        ),
        text,
    )
